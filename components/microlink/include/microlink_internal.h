...
#define ML_CTRL_PROTOCOL_VER    138
...
struct microlink_s {
    ...
    /* Advertised routes for subnet routing (JSON string, e.g. '["192.168.24.0/24"]').
     * Copied during init, sent in MapRequest. */
    char advertise_routes[128];
    volatile microlink_subnet_route_state_t subnet_route_state;
    bool enable_subnet_routing;  /* Reserved; Hostinfo.RoutableIPs is controlled by advertise_routes. */
